...
"""Qwen3.5-MoE routed experts. Dispatches decode vs prefill on the caller's explicit
mode (NOT the input shape): multi-user decode places the B users on dim-2, so a shape
test would misroute any B>1 decode into the prefill branch. decode_forward is written
for arbitrary batch on dim-2, so mode-based dispatch handles B>1 decode correctly."""
