...
def test_pow_zero_base_special_cases(device, dtype):
    # Shared generators strip ±0, and allclose(atol=5e-4) cannot see 2**(-127p).
    # Exact equality is required: today's wrong values (e.g. pow(0, 0.5) ≈ 7.67e-20) sit inside that tolerance.
    torch_dtype = getattr(torch, dtype)
    ttnn_dtype = getattr(ttnn, dtype)
    shape = [1, 1, 32, 32]
    ...
    def assert_zero_power_undefined(out):
        if dtype == "float32":
            ...
            # bf16 dest packs the NaN encoding as inf
