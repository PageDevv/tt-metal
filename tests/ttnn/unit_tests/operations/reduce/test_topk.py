...
# ttnn.experimental.topk_large_indices (values emitted directly by the op;
# layout conversions are conditional — TILE-native input/output where the
# op's writers support it, otherwise untilize -> op -> tilize; a slice pair
# only when k is not a multiple of 32), bypassing the single-core cliff (the
# device op's own multi-core path is gated at k <= 64 + pow2 width + width
# < 65536).
#
...
@pytest.mark.parametrize("W", [262144, 524288, 1048576])
@pytest.mark.parametrize("k", [1536, 2048])
def test_topk_large_k_routed_wide(k, W, device):
    # The routed envelope extends to 2^20 (prefill KV lengths). Fewer rows
    # keep host-side reference cost and transfer size sane at 1M width.
...
def test_topk_large_k_routed_neginf_lanes(device):
    # Rows whose top-k contains exact -inf: the routed path passes
    # neginf_sentinel=False to topk_large_indices, so -inf lanes carry their
    # REAL source positions (stock/torch parity), never the experimental op's
    # 0xFFFFFFFF sentinel. W=65536 is tile-aligned, so every returned index
    # must be a real in-range column.
    torch.manual_seed(2005)
...
def test_topk_routed_k_not_multiple_of_32(device):
    # k=100 exercises the routed k % 32 != 0 tail: the op runs at
    # k_rounded=128 and post_topk_transform slices back down to 100.
    run_topk_large_k_routed_test(1, 1, 32, 8192, 100, device)


@pytest.mark.skipif(not is_blackhole(), reason="MoE-gate routing arm is Blackhole-only")
def test_topk_moe_gate_routed(device):
    # The MoE-gate arm (k <= 16, padded width in [128, 512], <= 32 rows):
    # decode-class expert-gate shape, k not a multiple of 32 (op runs at
    # k_rounded=32, sliced back to 10).
    run_topk_large_k_routed_test(1, 1, 32, 512, 10, device)
