...
def test_topk_large_indices_neginf_sentinel_false_returns_real_positions(device, k):
    ...
    finite_values = torch.arange(finite_count, dtype=torch.float32).to(torch.bfloat16)
    torch_input[:, :finite_count] = finite_values
    tt_indices = ttnn.experimental.topk_large_indices(_to_device(torch_input, device), k=k, neginf_sentinel=False)
...
def test_topk_large_indices_neginf_sentinel_flag_in_program_cache(device):
    ...
    idx_sentinel = ttnn.to_torch(ttnn.experimental.topk_large_indices(tt_input, k=k, neginf_sentinel=True)).to(
        torch.int64
    )
    after_first = device.num_program_cache_entries()
    idx_real = ttnn.to_torch(ttnn.experimental.topk_large_indices(tt_input, k=k, neginf_sentinel=False)).to(torch.int64)
    ...
    idx_sentinel2 = ttnn.to_torch(ttnn.experimental.topk_large_indices(tt_input, k=k, neginf_sentinel=True)).to(
        torch.int64
    )
    idx_real2 = ttnn.to_torch(ttnn.experimental.topk_large_indices(tt_input, k=k, neginf_sentinel=False)).to(
        torch.int64
    )
