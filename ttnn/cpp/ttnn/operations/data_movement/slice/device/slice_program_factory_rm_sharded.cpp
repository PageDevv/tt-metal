    ...
    std::visit(
        [&](auto&& f) {
            ...
            if constexpr (std::is_same_v<Factory, SliceRmProgramFactory>) {
                ...
                // Reader/writer per-core scalars (start_id, num_tiles, id_per_dim, num_pages,
                // num_tiles_written) are work-split-derived and hash-excluded, so a same-hash hit on
                // a differently-partitioned cached program leaves writer num_pages=0 -> all-zero
                // output (#52651). Re-emit them alongside the address slots.
