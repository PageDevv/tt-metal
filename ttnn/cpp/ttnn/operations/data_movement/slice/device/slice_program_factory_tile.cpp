...
std::vector<tt::tt_metal::DynamicRuntimeArg> slice_tile_dynamic_args(
    ...
    // Must reproduce the per-core work split from create_descriptor exactly; any divergence would leave
    // stale scalars in the same slots this helper writes.
    const auto& input = tensor_args.input;
    tt::tt_metal::IDevice* device = input.device();
    ...
    for (const auto& core : cores) {
        ...
        // Writer slot 0 (dst buffer) is patched by patch_slot0 in patch_slice_program_addresses;
        // re-emit only slots 1 and 2 here.
