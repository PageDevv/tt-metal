...
tt::tt_metal::ProgramDescriptor UntilizeWithUnpaddingMultiCoreInterleavedProgramFactory::create_descriptor(
    ...
    // each row up to the buffer alignment - so it needs the padded stride, not the row size, or
    // every row after the first lands short. Interleaved outputs are unaffected: their accessor
    // re-aligns the page size itself (InterleavedAddrGen::aligned_page_size).
