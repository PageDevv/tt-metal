...
AllToAllAsyncGenericProgram::create_at(
    ...
    const auto physical_fabric_topology = tt::tt_fabric::get_fabric_topology();
    const bool fabric_has_wrap_links = physical_fabric_topology == tt::tt_fabric::Topology::Ring ||
                                       physical_fabric_topology == tt::tt_fabric::Topology::Torus;
    ...
    const auto connection_topology =
        operation_uses_wrap_links ? physical_fabric_topology : tt::tt_fabric::Topology::Linear;
    ...
    tt::tt_fabric::FabricMuxConfig mux_config(
        /*num_full_size_channels=*/mux_config_clients,
        /*num_header_only_channels=*/0,
        /*num_buffers_full_size_channel=*/num_mux_buffers_per_channel,
    ...
    std::set<CoreRange> active_mux_core_set;
    if (use_worker_mux) {
        for (uint32_t link = 0; link < operation_attributes.num_links; ++link) {
            if (sender_stream_direction_masks[0] != 0) {
                active_mux_core_set.emplace(mux_cores[link][0]);
            }
            if (sender_stream_direction_masks[workers_per_direction] != 0) {
                active_mux_core_set.emplace(mux_cores[link][1]);
            }
        }
    }
    tt::tt_metal::KernelHandle mux_kernel_id = 0;
    if (!active_mux_core_set.empty()) {
        mux_kernel_id = tt::tt_metal::CreateKernel(
            program,
            "tt_metal/fabric/impl/kernels/tt_fabric_mux.cpp",
            CoreRangeSet(active_mux_core_set),
    ...
    for (size_t stream = 0; stream < num_senders_per_link; ++stream) {
        ...
        if (stream_uses_mux) {
            ...
            sender_writer_kernel_config.compile_args.insert(
                sender_writer_kernel_config.compile_args.end(), {1, 0, 0, 0, 0});
        }
    ...
    if (use_worker_mux) {
        for (uint32_t link = 0; link < operation_attributes.num_links; ++link) {
            ...
            for (uint32_t direction = 0; direction < mux_num_directions; ++direction) {
                ...
                const auto mux_core = mux_cores[link][direction];
                auto mux_rt_args = mux_config.get_fabric_mux_run_time_args(
                    ...
                    program,
                    mux_core);
                tt::tt_metal::SetRuntimeArgs(program, mux_kernel_id, {mux_core}, mux_rt_args);
            }
        }
    }
    ...
    for (uint32_t core_id = 0; core_id < sender_worker_cores.size(); ++core_id) {
        ...
        if (stream_uses_mux) {
            ...
            const uint32_t termination_master_stream = direction * workers_per_direction;
            const auto mux_virtual_core = device->worker_core_from_logical_core(mux_cores[link][direction]);
            const auto termination_master_virtual_core = device->worker_core_from_logical_core(
                sender_worker_cores[link * num_senders_per_link + termination_master_stream]);
            ttnn::ccl::fabric_mux_connection_rt_args(
                /*mux_connection_valid=*/true,
                /*is_termination_master=*/worker == 0,
                tt::tt_fabric::FabricMuxChannelType::FULL_SIZE_CHANNEL,
                mux_virtual_core,
                worker,
                core,
