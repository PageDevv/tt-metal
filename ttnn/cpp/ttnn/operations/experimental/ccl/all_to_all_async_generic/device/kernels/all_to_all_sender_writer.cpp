...
static_assert(!use_worker_mux || num_mux_clients > 0, "A fabric mux connection requires at least one client");
...
using FabricMuxSender = tt::tt_fabric::WorkerToFabricMuxSender<fabric_mux_num_buffers_per_channel>;
struct FabricMuxConnection {
    FabricMuxSender sender;
    uint8_t mux_x;
    uint8_t mux_y;
    uint32_t local_status_address;
    bool is_termination_master;
    uint32_t termination_sync_semaphore_id;
...
inline void finish_open_connections(FabricMuxConnection& fabric_connection) {
    tt::tt_fabric::wait_for_fabric_endpoint_ready(
        fabric_connection.mux_x,
        fabric_connection.mux_y,
        fabric_mux_status_address,
        fabric_connection.local_status_address);
    tt::tt_fabric::fabric_client_connect(fabric_connection.sender);
}
...
inline void close_connections(FabricMuxConnection& fabric_connection) {
    while (fabric_connection.sender.get_num_free_write_slots() != fabric_mux_num_buffers_per_channel) {
    }
    tt::tt_fabric::fabric_client_disconnect(fabric_connection.sender);

    Noc noc;
    Semaphore<> termination_sync(fabric_connection.termination_sync_semaphore_id);
    if (fabric_connection.is_termination_master) {
        termination_sync.wait(num_mux_clients - 1);
        tt::tt_fabric::fabric_endpoint_terminate(
            fabric_connection.mux_x, fabric_connection.mux_y, fabric_mux_termination_signal_address);
    } else {
        termination_sync.up(
            noc, fabric_connection.termination_master_noc_x, fabric_connection.termination_master_noc_y, 1);
...
void send_initialization(
    ...
    if constexpr (Topology == tt::tt_fabric::Topology::Ring) {
        ...
        for (uint32_t did = 0; did < local_num_devices; ++did) {
            ...
            set_route(packet_header, route_info, abs(device_offset), fabric_connection);
            ...
            fabric_connection.sender.send_payload_flush_blocking_from_address(
                packet_header_address, sizeof(PacketHeader));
        }
    } else {
        // Linear uses one multicast per direction, so only one mux client owns initialization.
        constexpr uint32_t compile_safe_num_mux_clients = num_mux_clients == 0 ? 1 : num_mux_clients;
        if (core_id % compile_safe_num_mux_clients != 0) {
            return;
        }
...
void kernel_main() {
    ...
    uint8_t termination_master_noc_x = 0;
    uint8_t termination_master_noc_y = 0;
    ...
    auto fabric_connections = [&]() {
        if constexpr (use_worker_mux) {
            auto sender = tt::tt_fabric::build_connection_to_fabric_endpoint<fabric_mux_num_buffers_per_channel>(
                fabric_mux_x,
                fabric_mux_y,
            ...
            return FabricMuxConnection{
                .sender = sender,
                .mux_x = fabric_mux_x,
                .mux_y = fabric_mux_y,
                .local_status_address = local_fabric_mux_status_address,
                .is_termination_master = is_termination_master,
                .termination_sync_semaphore_id = termination_sync_semaphore_id,
                .termination_master_noc_x = termination_master_noc_x,
                .termination_master_noc_y = termination_master_noc_y};
    ...
    {
        DeviceZoneScopedN("A2A_W_DATA");
        for (uint32_t did = 0; did < local_num_devices; ++did) {
            const int32_t device_offset = get_arg_val<int32_t>(device_offsets_idx++);
            const int32_t distance = abs(device_offset);
    ...
    noc_obj.async_write_barrier();
    {
        DeviceZoneScopedN("A2A_W_CLOSE");
        if constexpr (has_fabric_connections) {
            close_connections(fabric_connections);
        }
    }
    if (core_id == 0 && link_id == 0) {
        DeviceZoneScopedN("A2A_W_FINAL_WAIT");
