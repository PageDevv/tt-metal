...
[[noreturn]] FORCE_INLINE void fail_stop_invalid_fabric_route() {
    // ASSERT is intentionally compiled out in production kernels. An invalid connection/route must stop this worker
    // instead of silently injecting a packet on an arbitrary egress and corrupting the collective's completion count.
    asm volatile("ebreak");
    while (true) {
    }
}
...
void send_initialization(
    ...
    if constexpr (Topology == tt::tt_fabric::Topology::Ring || is_fabric_2d) {
        ...
        // Fabric1D Linear uses one multicast per direction, so only one Mux V2 client owns initialization.
...
void send_initialization(
    ...
    uint32_t local_num_devices,
    size_t device_offsets_idx,
    [[maybe_unused]] uint64_t init_semaphore_noc_addr_in_pkt,
    uint32_t global_init_semaphore_addr,
    volatile PacketHeader* pkt_hdr_sema_forward,
    volatile PacketHeader* pkt_hdr_sema_backward,
    uint32_t packet_header_buffer_addr_sema_forward,
...
void write_data(
    ...
    if (local) {
        ...
        if (last) {
            ...
            if (parts_count > 1) {
                if (parts_count > 2) {
                    ...
                    perform_payload_send(
                        select_connection(
                            fabric_connections, device_offset, dest_mesh_id, dest_chip_id, initial_direction),
                        l1_read_addr,
                        scatter_payload,
                        pkt_hdr);
            ...
            perform_payload_send(
                select_connection(fabric_connections, device_offset, dest_mesh_id, dest_chip_id, initial_direction),
                l1_read_addr,
                payload_sizes[parts_count - 1],
                pkt_hdr);
        } else {
