...
The plan's Option A verbatim, on **all three arches** — the plan asked for Quasar to be verified rather than
assumed, and it has the same defect:
...
No register-allocator ICE on any arch (the plan's §8 flagged that risk).
...
| 5 | **Blackhole and Quasar are compile-verified only** | No BH or Quasar hardware on this host. Plan §7 step 3 wants the edge sweep run on both. |
