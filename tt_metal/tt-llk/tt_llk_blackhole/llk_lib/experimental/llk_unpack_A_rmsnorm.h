    ...
    [[maybe_unused]] const std::uint32_t unpack_dst_format = 0)
{
    ...
    // Only these two are reachable. The static_asserts above pin this MOP to
    // SCALAR + acc_to_dest + DEST_TO_SRCB, so SrcB is never unpacked from L1 -- it is
    // filled by MOVD2B on the math thread and only needs the dummy dvalid below. The
    // unpack-to-dest and SrcB-stepping opcodes carried over from llk_unpack_A.h's general
    // mop_config were unreachable here and are dropped; they tripped
    // -Werror=unused-variable the moment a tt-llk test first compiled this header.
